...
// Two-parent form that takes a workspace arena. The T2_G128_FP16 (folded ternary) route uses the
// arena for its rotation scratch; every other parent route behaves exactly like the
// stream-only overload above.
void attn_input_proj(const Tensor& x, const Weight& query_key_weight,
                     const Weight& gate_value_weight, Tensor& q, Tensor& gate, Tensor& k, Tensor& v,
