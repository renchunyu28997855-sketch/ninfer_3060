...
Parameters::Parameters(const Model& source) : model(source) {
    ...
    // The artifact stores the embedding table folded into the rotated basis (GGUF
    // inverse_weight_names covers it) but its binding carries no auxiliary, so attach the
    // shared width-matched sign table here; without it the gathered rows stay rotated and
    // every downstream number is garbage.
    auto token_embedding = native_weight(model.weight(w.text.token_embedding).view);
    if (token_embedding.qtype == QType::T2_G128_FP16) {
        const std::size_t width = model.config().text.hidden_size;
        const auto* signs = model.hadamard_signs_for(width);
        if (signs != nullptr) {
            token_embedding.hadamard_signs = signs;
            token_embedding.hadamard_n_blk = static_cast<std::int32_t>(width / 1024);
        }
    }
