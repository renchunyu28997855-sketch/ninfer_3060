...
std::string format_throughput_json(const std::string& server_instance_id, std::uint64_t timestamp,
    ...
    record["context_cache"] = Json{
        ...
        {"working_set",
         Json{{"selections",
               monotonic_delta(previous.working_set_selections, current.working_set_selections)},
              {"swaps", monotonic_delta(previous.working_set_swaps, current.working_set_swaps)},
              {"demoted_pages",
               monotonic_delta(previous.working_set_demoted_pages,
                               current.working_set_demoted_pages)},
              {"promoted_pages",
               monotonic_delta(previous.working_set_promoted_pages,
                               current.working_set_promoted_pages)},
              {"d2h_bytes",
               monotonic_delta(previous.working_set_d2h_bytes, current.working_set_d2h_bytes)},
              {"h2d_bytes",
               monotonic_delta(previous.working_set_h2d_bytes, current.working_set_h2d_bytes)},
              {"d2h_seconds",
               monotonic_delta(previous.working_set_d2h_seconds,
                               current.working_set_d2h_seconds)},
              {"h2d_seconds",
               monotonic_delta(previous.working_set_h2d_seconds,
                               current.working_set_h2d_seconds)},
              {"selection_seconds",
               monotonic_delta(previous.working_set_selection_seconds,
                               current.working_set_selection_seconds)}}},
        {"pressure",
         Json{
             {"spill_pages",
