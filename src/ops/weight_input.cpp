...
SingleProjectionWeight single(std::span<const WeightInput> inputs) {
    ...
    auto weight = native_weight(view, divisor);
    // Folded-ternary rotation metadata: every row of a joined parent shares the input width, so
    // any carrying input supplies the table for the whole view.
    const WeightInput* signs_source = nullptr;
    for (const auto& input : inputs) {
        if (input.hadamard_signs != nullptr) { signs_source = &input; break; }
    }
    if (signs_source != nullptr) {
        weight.hadamard_signs   = signs_source->hadamard_signs;
        weight.hadamard_n_blk   = signs_source->hadamard_n_blk;
        weight.hadamard_perm_hd  = signs_source->hadamard_perm_hd;
        weight.hadamard_perm_nk  = signs_source->hadamard_perm_nk;
        weight.hadamard_perm_rep = signs_source->hadamard_perm_rep;
    }
...
ProjectionWeights input_projection(std::span<const WeightInput, 4> inputs, bool attention) {
    ...
    const bool paired_int   =
        first.weight.qtype == QType::Q4_G64_FP16 && second.weight.qtype == QType::Q5_G64_FP16;
    const bool paired_ternary =
        first.weight.qtype == QType::T2_G128_FP16 && second.weight.qtype == QType::T2_G128_FP16;
    require(paired_int || paired_ternary,
            "input projection: paired native form requires Q4/Q5 or folded ternary (T2)");
