...
// GA10x shared-memory budget. The medium-T grouped tiles exceed the 48 KB static
// cap on sm_86/sm_89; on those targets the buffers come from dynamically opted-in
// shared memory instead (sm_120a keeps the static arrays).
template <int TileCols, int KSplits, int NGroups>
struct Q8GroupedSharedBytes {
    static constexpr int kCodeBytes   = 16 * (KSplits * 64);
    static constexpr int kCodeAligned  = (kCodeBytes + 15) & ~15;
    static constexpr int kActivBytes   = KSplits * NGroups * (TileCols / NGroups) * 64 * 2;
    static constexpr int value         = kCodeAligned + kActivBytes;
};

inline void q8_configure_optin_shared_mem(const void* kernel, int bytes)
{
    CUDA_CHECK(cudaFuncSetAttribute(kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, bytes));
    CUDA_CHECK(cudaFuncSetAttribute(kernel, cudaFuncAttributePreferredSharedMemoryCarveout, 100));
}
...
__global__ __launch_bounds__(KSplits* NGroups * 32, MinBlocks) void q8_ksplit_grouped_mma_kernel(
    ...
    auto* code_shared =
        reinterpret_cast<std::uint8_t(*)[kMmaRows][kGroupK]>(raw_shared);
    auto* b_shared = reinterpret_cast<__nv_bfloat16(*)[kKernelWarps][kWarpCols * kTileK]>(
        raw_shared + Q8GroupedSharedBytes<TileCols, KSplits, NGroups>::kCodeAligned);
#else
