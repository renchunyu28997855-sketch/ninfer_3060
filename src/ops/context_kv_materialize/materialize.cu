...
template <int Columns, int KWarps = 8>
__global__ __launch_bounds__(GroupedSchedule<Columns, KWarps>::kThreads, 1) void context_kv_grouped_kernel(
...
void launch_grouped(const Tensor& x, const Tensor& positions, const Tensor& counts,
    ...
    context_kv_grouped_kernel<Columns, KWarps>
        <<<dim3(64, (envelope.max_count * x.ne[2] + Columns - 1) / Columns, 10),
           GroupedSchedule<Columns, KWarps>::kThreads, 0, stream>>>(static_cast<const __nv_bfloat16*>(x.data),
